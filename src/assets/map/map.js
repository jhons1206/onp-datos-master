(function() {
	...
	d.handleWheel = function(a) {
		for (var b = d.charts, c = 0; c < b.length; c++) {
			var e = b[c];
			if (e && e.mouseIsOver) { (e.mouseWheelScrollEnabled || e.mouseWheelZoomEnabled) && e.handleWheel && (e.handleMouseMove(a), e.handleWheel(a));
				break
			}
		}
	};
	...
	d.addGlobalListeners = function() {
		...
		document.addEventListener("mousemove", d.handleMouseMove), document.addEventListener("keyup", d.handleKeyUp), document.addEventListener("mouseup", d.handleMouseUp, !0), window.addEventListener("load", d.handleLoad, !0)), d.isIE && (document.attachEvent("onmousemove", d.handleMouseMove), document.attachEvent("onmouseup", d.handleMouseUp), window.attachEvent("onload", d.handleLoad)))
	};
...
(function() {
	...
	d.formatDuration = function(a, b, c, e, g, f) {
		...
		c = a + "" + e[b] + "" + c;
		if (k[g].count > k[b].count) for (a = k[b].count; a < k[g].count; a++) b = k[b].nextInterval,
...
(function() {
	...
	d.SVGRenderer = d.Class({
		...
		update: function() {
			...
			for (a = b.length - 1; 0 <= a; a--) {
				...
				if (g <= e) {
					...
					this.setAttr(f, k, c)
				} else "translate" == k ? (l = c.to.split(","), m = Number(l[0]), l = Number(l[1]), f.translate(m, l)) : (h = Number(c.to), this.setAttr(f, k, h)),
		...
		gradient: function(a, b, c, e) {
			...
			for (e = 0; e < c.length; e++) k = document.createElementNS(d.SVG_NS, "stop"),
			h = 100 * e / (c.length - 1),
			0 === e && (h = 0),
...
(function() {
	...
	d.AmChart = d.Class({
		...
		makeAccessible: function(a, b, c) {
			this.accessible && a && (c && a.setAttr("role", c), a.setAttr("aria-label", b.trim()))
		},
		...
		brr: function() {
			if (!this.hideCredits) {
				...
				if (c != a || !0 === this.inIframe()) {
					...
					"ammap" == this.product && (c = a + "/javascript-maps/", b = "", e = "");
...
(function() {
	...
	d.polygon = function(a, b, c, e, g, f, k, h, l, m, n) {
		...
		m && (k = Number);
		h = "M" + (k(b[0]) + n) + "," + (k(c[0]) + f);
		for (p = 1; p < b.length; p++) m && (b[p] = d.roundTo(b[p], 5), c[p] = d.roundTo(c[p], 5)),
		h += " L" + (k(b[p]) + n) + "," + (k(c[p]) + f);
		a = a.path(h + " Z").attr(g);
	...
	d.line = function(a, b, c, e, g, f, k, h, l, m, n) {
		...
		g = d.dy;
		k = "M" + (e(b[0]) + m) + "," + (e(c[0]) + g);
		for (h = 1; h < b.length; h++) b[h] = d.roundTo(b[h], 5),
		c[h] = d.roundTo(c[h], 5),
		k += " L" + (e(b[h]) + m) + "," + (e(c[h]) + g);
	...
	d.makeHD = function(a, b, c) {
		for (var e = [], d = [], f = 1; f < a.length; f++) for (var k = Number(a[f - 1]), h = Number(b[f - 1]), l = Number(a[f]), m = Number(b[f]), n = Math.round(Math.sqrt(Math.pow(l - k, 2) + Math.pow(m - h, 2)) / 50) + 1, l = (l - k) / n, m = (m - h) / n, p = 0; p <= n; p++) {
	...
	d.AmLegend = d.Class({
		...
		arrangeEntries: function() {
			...
			for (w = 0; w < u.length; w++) {
				...
				E.translate(F, x);
				A++; ! isNaN(l) && A >= l && (A = 0, t++, x = x + z + m, C = f, z = 0);
				B.push(E)
			}
			v = B.getBBox();
			l = v.height + 2 * m - 1;
		...
		createEntry: function(a) {
			if (!1 !== a.visibleInLegend && !a.hideFromLegend) {
				...
				var h; ! 0 === a.hidden && (h = f = b.markerDisabledColor);
				var l = a.pattern,
				m, n = a.customMarker;
				n || (n = b.customMarker);
				var p = b.container,
				...
				if (e) {
					e = a.type;
					b.switchType = void 0;
					if ("line" == e || "step" == e || "smoothedLine" == e || "ohlc" == e) m = p.set(),
					...
					m.push(t),
					a.bullet && (a.hidden || (f = a.bulletColorR), t = d.bullet(p, a.bullet, a.bulletSize, f, a.bulletAlpha, a.bulletBorderThickness, h, a.bulletBorderAlpha)) && (d.setCN(c, t, "graph-bullet"), t.translate(r + 1, r / 2), m.push(t)),
					y = 0,
					t = r,
					q = r / 3;
					else {
						a.getGradRotation && (m = a.getGradRotation(), 0 === m && (m = 180));
						t = a.fillColorsR; ! 0 === a.hidden && (t = f);
						if (m = b.createMarker("rectangle", t, a.fillAlphas, a.lineThickness, f, a.lineAlpha, m, l, a.dashLength)) y = r,
						m.translate(y, r / 2);
						t = r
					}
					d.setCN(c, m, "graph-" + e);
				...
				b.useMarkerColorForLabels && !l && (h = f); ! 0 === a.hidden && (h = b.markerDisabledColor);
		...
		createValue: function(a) {
			...
			if (!1 !== a.visibleInLegend && !a.hideFromLegend) {
				...
				if (b.valueText && 0 < b.valueWidth) {
					var l = b.color;
					b.useMarkerColorForValues && (l = a.color, a.legendKeyColor && (l = a.legendKeyColor())); ! 0 === a.hidden && (l = b.markerDisabledColor);
					var m = b.valueText,
...
(function() {
	...
	d.AmMap = d.Class({
		...
		initChart: function() {
			...
			else(d.AmMap.base.initChart.call(this), d.hasSVG) ? (this.dataChanged && (this.parseData(), this.dispatchDataUpdated = !0, this.dataChanged = !1, a = this.legend) && (a.position = "absolute", a.invalidateSize()), this.createDescriptionsDiv(), this.svgAreas = [], this.svgAreasById = {},
			this.drawChart()) : (this.chartDiv.style.textAlign = "", this.chartDiv.setAttribute("class", "ammapAlert"), this.chartDiv.innerHTML = this.svgNotSupported, this.fire({
				type: "failed",
				chart: this
			}))
		},
		...
		buildEverything: function() {
			if (0 < this.realWidth && 0 < this.realHeight) {
				...
				if (c = this.smallMap) c = d.processObject(c, d.SmallMap, this.theme),
				c.init(this, a),
				this.smallMap = c;
		...
		rescaleObjects: function() {
			...
			if (e = b.linesToResize) for (c = 0; c < e.length; c++) d = e[c],
				...
				if (a = b.mapContainer) {
					...
					if (!isNaN(h)) {
						...
						if (!isNaN(q)) {
							...
							l = (b.zoomY() - l * t) * (h / k) + l * t;.1 < Math.abs(h - k) && (b.zoomTo(h, q, l, !0), b.mapWasPinched = !0, clearTimeout(b.pinchTO))
						}
					}
					h = a.scale;
		...
		selectObject: function(a, b) {
			...
			e = c.zoomDuration; ! f && isNaN(a.zoomLevel) && isNaN(a.zoomX) && isNaN(a.zoomY) ? (c.showDescriptionAndGetUrl(), b || c.processObjects()) : (c.selectedObjectTimeOut = setTimeout(function() {
			...
			if (e) {
				if (d.removeCN(c, e, "selected-object"), d.setCN(c, e, "selected-object"), a.bringForwardOnHover && a.displayObject.toFront(), c.outlinesToFront(), !a.preserveOriginalAttributes) {
					...
					if (l = a.imageLabel) m = a.selectedLabelColorReal,
		...
		showAsRolledOver: function(a) {
			...
			if (!a.showAsSelected && b && !a.isOver) {
				...
				b.node.onclick = function() {}; ! a.isFirst && a.bringForwardOnHover && (b.toFront(), a.isFirst = !0);
				...
				if (void 0 != c) if ("MapImage" == a.objectType)(e = a.image) && e.setAttr("fill", c);
				else if ("MapLine" == a.objectType) {
					if ((e = a.lineSvg) && e.setAttr("stroke", c), e = a.arrowSvg) e.setAttr("fill", c),
					e.setAttr("stroke", c)
			...
			if (c) {
				...
				for (g = 0; g < h; g++) {
					...
					if (0 !== f.childNodes.length || "" !== m || !1 !== n) if (3 == f.nodeType || 4 == f.nodeType) {
						...
						var p;
						void 0 !== a[b][l] ? void 0 === a[b][l].length ? (p = a[b][l], a[b][l] = [], a[b][l].push(p), a[b][l].push({}), p = a[b][l][1]) : "object" == typeof a[b][l] && (a[b][l].push({}), p = a[b][l][a[b][l].length - 1]) : (a[b][l] = {},
						p = a[b][l]);
		...
		fixMapPosition: function() {
			...
			c = f = Infinity;
			for (e = 0; e < this.svgAreas.length; e++) g = this.svgAreas[e].getBBox(),
			g.y < f && (f = g.y),
			g.x < c && (c = g.x);
		...
		xyToCoordinates: function(a, b, c, e, g) {
			...
			} else f = this.coordinateToLongitude(a),
...
(function() {
	...
	d.AreasProcessor = d.Class({
		...
		process: function(a) {
			...
			b.minValueReal = h;
			for (d = 0; d < c; d++) g = a[d],
			isNaN(g.value) ? g.percents = void 0 : (g.percents = (g.value - h) / f * 100, h == k && (g.percents = 100));
			for (d = 0; d < c; d++) g = a[d],
			this.createArea(g);
			b.outlinesToFront()
		},
...
(function() {
	...
	d.ImagesProcessor = d.Class({
		...
		createImage: function(a, b) {
			...
			G = a.labelColorReal; ! a.showAsSelected && c.selectedObject != a || void 0 == k || (D && D.setAttr("fill", k), G = a.selectedLabelColorReal);
			D = null;
		...
		updateSizeAndPosition: function(a) {
			...
			else if (!isNaN(l) && !isNaN(h)) if (g = b.coordinatesToXY(h, l), e = g.x, g = g.y, isNaN(p) || (e = p), isNaN(r) || (g = r), a.fixedSize) {
				p = a.positionScale;
	...
	d.LinesProcessor = d.Class({
		...
		createLine: function(a, b) {
			...
			if (a.remainVisible || c.selectedObject == a.parentObject) {
				...
				e = a.latitudes;
				q = n.length;
				if (0 < q) for (w = [], C = [], u = 0; u < q; u++) B = c.coordinatesToXY(n[u], e[u]),
				w.push(B.x),
				C.push(B.y);
				if (0 < w.length) {
					a.segments = w.length;
					...
					if (isNaN(q)) {
						for (q = 0; q < w.length - 1; q++) H = Math.sqrt(Math.pow(w[q + 1] - w[q], 2) + Math.pow(C[q + 1] - C[q], 2)),
						a.distances[q] = H;
					...
					if ("none" != A) {
						var I, K, N;
						if ("end" == A || "both" == A) u = w[w.length - 1],
						D = C[C.length - 1],
	...
	d.ValueLegend = d.Class({
		...
		init: function(a, b) {
			if (this.enabled) {
				...
				if (this.showAsGradient) c = d.rect(b, this.width, this.height, [c, e], 1, this.borderThickness, this.borderColor, 1, 0, 0),
				d.setCN(a, c, "value-legend-gradient"),
				c.translate(0, k),
				f.push(c);
				else for (l = this.width / g, m = 0; m < g; m++) n = d.getColorFade(c, e, 1 * m / (g - 1)),
...
(function() {
	...
	d.ObjectList = d.Class({
		...
		addObjects: function(a, b) {
			...
			if (a.images) for (g = 0; g < a.images.length; g++) f = a.images[g],
			void 0 === f.showInList && (f.showInList = c.showImagesInList),
			this.addObject(f, d);
			if (a.lines) for (g = 0; g < a.lines.length; g++) f = a.lines[g],
